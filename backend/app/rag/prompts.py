...
输出格式要求：
1. 使用 Markdown 输出，至少包含以下小节：
   - `## 结论`
   - `## 依据与分析`
   - `## 局限与不确定性`
   - `## 参考来源`
2. 在“依据与分析”中，每个关键论断至少附一个引用编号，如 [1]。
3. “参考来源”按 `[编号] 简要标题或主题` 列表输出，编号需与正文引用一致。
4. 若没有可用资料，明确写出“未检索到可支撑该结论的参考资料”，不要编造引用。"""
...
请基于以上参考资料回答用户问题。如果参考资料不足以完整回答，可以结合你的知识进行补充，但需注明哪些是基于资料、哪些是补充说明。回答时使用[1][2]等格式标注引用来源，并严格遵守系统角色中的输出格式。"""
