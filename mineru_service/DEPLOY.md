# MinerU 解析服务部署指南 (Linux 裸机 + Docker)
...
**v2 更新**: 当前仓库同时保留两条可用路径:

- Linux 4090 裸机启动: `requirements.native.txt` + `start_native.sh`
...
| 环境 | MinerU 服务 | 主后端 |
|------|------------|--------|
| Linux 4090 服务器 | 裸机启动, 端口 8010 | `MINERU_API_URL=http://<4090-ip>:8010` |
...
这一条路径面向你要租用的 4090 Linux 服务器。核心原则是: 不依赖 Docker，直接在宿主机上安装 Python 运行时、MinerU 核心依赖和模型缓存，然后使用 `start_native.sh` 启动同样的 FastAPI 服务。`start_linux.sh` 只是一个同义入口，最终会转到 `start_native.sh`。
...
- 4090 服务器可访问 HuggingFace 或对应镜像源
...
这组参数与仓库中的 `start_native.sh` 默认值一致，优先发挥 4090 的 24GB 显存。若你发现公式密集型文档仍然容易触发显存抖动，再把 `PIPELINE_DEVICE` 临时调回 `cpu`。
...
Environment=GPU_MEMORY_UTILIZATION=0.60
Environment=PIPELINE_DEVICE=cuda
Environment=MAX_CONCURRENT=2
Environment=MAX_QUEUE_SIZE=8
